...
// namespace CopperTag{
  ...
  void adaptive_DP(std::vector<cv::Point> &pts, std::vector<cv::Point> &DominantPoints)
  {
  ...
  void AAMED::aamed_poly_DP(std::vector<std::vector<cv::Point>> &edgeSegments, 
    std::vector<std::vector<cv::Point>> &dpSegments)
  {
    dpSegments.clear();
    for (int i = 0; i < edgeSegments.size(); i++){
      std::vector<cv::Point> dpContour; 
      adaptive_DP(edgeSegments[i], dpContour);
      dpSegments.push_back(dpContour);
    }
  }
// }